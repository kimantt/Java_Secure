...
<head>
    ...
    <script type="text/javascript">
		// 서버에서 해석해야 할 내용이 없는 공격, DOM based 공격에 취약한 코드가 될 수 있음
		const hash = window.location.hash.slice(1); // # 뒤에 문자열이 있으면 해당 문자열 반환
		                                            // slice(idx) idx부터 끝까지 추출 hash.slice(1) -> hash 문자열이 #first.jsp면 first.jsp 반환
		if (hash) { // first.jsp -> 일반 문자열(url 형식으로 구성해야 href에 대입될 수 있음)
